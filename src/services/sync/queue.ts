...
import { scheduleDebouncedSync } from './debouncedSync'

let syncTrigger: (() => void | Promise<void>) | null = null

export function registerSyncTrigger(trigger: () => void | Promise<void>): void {
  syncTrigger = trigger
}
...
function requestSyncAfterChange(): void {
  if (!syncTrigger) return
  scheduleDebouncedSync(syncTrigger)
}
